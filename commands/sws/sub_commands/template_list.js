...
if (DEBUG_MODE) {
    console.log(chalk.blue('[DEBUG] Initializing template list command...'))
} else {
    console.log('[INFO] Initializing template list command...')
}
...
module.exports = {
    async execute(interaction) {
        try {
            ...
            // テンプレートが格納されているベースディレクトリを取得
            const templateBaseDir = config.templateBasePath;
            let entries;
            try {
                entries = await fs.readdir(templateBaseDir, { withFileTypes: true })
                if (DEBUG_MODE) {
                    console.log(chalk.blue('[DEBUG] Fetching template list...'))
                    console.log(chalk.green(`[DEBUG] Found ${entries.length} templates.`))
                }
            } catch (readDirError) {
                console.error(chalk.red(`Failed to read template directory: ${templateBaseDir}`, readDirError))

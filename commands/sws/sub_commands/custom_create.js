...
const utils = require('../../../escape/utils')
...
if (DEBUG_MODE) {
    console.log(chalk.blue('[DEBUG] Initializing custom create command...'))
} else {
    console.log('[INFO] Initializing custom create command...')
}
...
module.exports = {
    async execute(interaction) {
        ...
        if (DEBUG_MODE) {
            console.log(chalk.blue(`[DEBUG] Creating custom server configuration: ${configName}`))
        }

        const logThread = null // ログスレッドの初期化

        try {
            // --- 基本チェック ---
            if (!utils.isValidConfigName(configName)) {
                await interaction.reply({ content: messages.get('ERROR_CONFIG_NAME_INVALID', { invalidName: configName }), ephemeral: false })
                return
            }
            if (configFileAttachment.contentType !== 'text/xml' && configFileAttachment.name !== 'server_config.xml') {
                 await interaction.reply({ content: `エラー: 添付ファイルは 'server_config.xml' という名前のXMLファイルである必要があります。\n(ファイル名: ${configFileAttachment.name}, タイプ: ${configFileAttachment.contentType || '不明'})`, ephemeral: false })
                return
            }
            const configExists = await utils.checkConfigExists(configName)
            if (configExists) {
                await interaction.reply({ content: messages.get('ERROR_CONFIG_ALREADY_EXISTS', { configName }), ephemeral: false })
                return
            }

            await interaction.deferReply({ ephemeral: false })

            log('INFO', `カスタムサーバー構成 "${configName}" の作成を開始します。`, { interaction, thread: logThread })
            ...
            try {
                ...
                await interaction.editReply({ content: `添付ファイルのダウンロードに失敗しました: ${downloadError.message}`, ephemeral: false })
                return
            }

            const validationResult = await checkConfig.validateServerConfig(xmlDataString)
            ...
            if (availablePort === null) {
                ...
                await interaction.editReply({ content: messages.get('ERROR_PORT_NOT_AVAILABLE', { minPort: config.minPort, maxPort: config.maxPort }), ephemeral: false })
                return
            }
            console.log(`[INFO] Assigning an available port to ${configName}.`)
            ...
            try {
                 ...
                 await interaction.editReply({ content: `構成ディレクトリの作成に失敗しました: ${mkdirError.message}`, ephemeral: false })
                 return
            }
            ...
            // 4d. XML更新と保存 (ポート番号)
            await interaction.editReply({ content: `シンボリックリンク作成完了。設定ファイルにポート番号を割り当て中...`, ephemeral: false })
            try {
            ...
            log('ERROR', `カスタムサーバー構成 "${configName}" の作成中にエラーが発生しました。`, { interaction, error, thread: logThread })
        }

        if (DEBUG_MODE) {
            console.log(chalk.green(`[DEBUG] Successfully created custom configuration: ${configName}`))
        }
    }
}
 ...
 * @returns {Promise<{ allReady: boolean, results: {id: string, success: boolean, message: string, targetPath: string}[] }>}
 */
async function prepareWorkshopItems(requiredItems, interaction) {
    if (requiredItems.size === 0) {
        console.log('[WorkshopDownloader] No workshop items required.');
        return { allReady: true, results: [] };
    }

    await interaction.followUp({ content: `**${requiredItems.size}個** のワークショップアイテムの準備を開始します... (存在確認とダウンロード)`, ephemeral: false });
    ...
    for (const [id, itemInfo] of requiredItems.entries()) {
        ...
        try {
            if (interaction.channel) {
                await interaction.channel.send(progressMsg);
            } else {
                console.log(`[WorkshopDownloader] Cannot send progress message: interaction.channel is null.`);
            }
        } catch (sendError) {
            console.error(`[WorkshopDownloader] Failed to send progress message:`, sendError);
        }

        const exists = await checkItemExists(id); // 存在確認はIDで行う
        if (exists) {
            console.log(`[WorkshopDownloader] Item ${id} already exists. Skipping download.`);
            results.push({ id, success: true, message: `☑️ ID \`${id}\`: 既に存在します`, targetPath });
            continue;
        }

        console.log(`[WorkshopDownloader] Item ${id} not found. Attempting download...`);
        const downloadResult = await downloadSingleItem(id);
        results.push({ id, success: downloadResult.success, message: downloadResult.message, targetPath });

        if (!downloadResult.success) {
            allReady = false; // 一つでも失敗したら全体として失敗
        } else {
             // ダウンロード成功後にもう一度存在確認を行う
             const existsAfterDownload = await checkItemExists(id);
             if (!existsAfterDownload) {
                 console.error(`[WorkshopDownloader] CRITICAL: Item ${id} reported as downloaded but not found at ${targetPath}!`);
                 results[results.length - 1].success = false; // 最終結果を失敗にする
                 results[results.length - 1].message = `❌ ID \`${id}\`: ダウンロード成功報告がありましたが、ファイルが見つかりません。`;
                 allReady = false;
             }
        }
    }
    ...
    const embed = new EmbedBuilder()
        .setTitle('ワークショップアイテム準備結果')
        .setColor(allReady ? 0x00FF00 : 0xFF0000)
        .setDescription(results.map(r => r.message).join('\n') || '対象アイテムなし')
        .setFooter({ text: `確認/ダウンロード先: ${WORKSHOP_CONTENT_PATH}` });

    await interaction.followUp({ embeds: [embed], ephemeral: false });
    console.log(`[WorkshopDownloader] Finished preparing items. All ready: ${allReady}`);
...
async function createPlaylistSymlinks(configName, preparedItemsMap, interaction) {
    ...
    for (const [id, itemInfo] of playlistItems) {
        ...
        try {
            ...
            // シンボリックリンク作成 ('junction' 推奨)
            await fs.symlink(targetPath, linkPath, 'junction');
            // ★ デバッグモードに関わらずログ出力（要件）
            console.log(`[Symlink] Successfully created link for ID ${id}.`);
